...
def ablate_max_orate(model_ckpts, data_loader, cache_dir, oracle=False, debug=False, overwrite=False):
...
def ablate_archs(model_ckpts, data_loader, cache_dir, oracle=False, debug=False, overwrite=False, ransac_thres=0.001, iterations_count=1000,
                confidence=0.99, split=None):
    if not os.path.exists(cache_dir):
        os.makedirs(cache_dir)
    if oracle:
        model_ckpts = [None]
    ...
    for ckpt_path in model_ckpts:        
        ...
        # Eval
        evaluator = MatcherEvaluator(ckpt_path=ckpt_path, oracle=oracle, ransac_thres=ransac_thres, iterations_count=iterations_count, confidence=confidence)
        data_loader.dataset.p3d_type = evaluator.p3d_type  
        evaluator.eval_data_loader(data_loader, debug=debug)
...
def run_ablation(model_dir, odir, model_pattern, root_dir='.', oracle=False, debug=False, ablate_orate=False, overwrite=False):
    if oracle:
        model_ckpts = [None]
    else:
        # Load ckpts
        model_ckpts = glob.glob(
            os.path.join(root_dir, model_dir, model_pattern)
        )
        if ablate_orate:
            # Models of interest
    ...
    ablate_fn(model_ckpts, data_loader, cache_dir=cache_dir, oracle=oracle, debug=debug, overwrite=overwrite)
