...
def eval_model(data_loader, cache_dir, ckpt_path=None, vismatch=False, oracle=False, 
               covis_k_nums=[1, 3, 5, 7, 10], debug=False, overwrite=False, prefix=None):
    if oracle:
        logger.info(f"\n\n\n>>>>>Oracle Matching")
        ...
        logger.info(f"\n\n\n>>>>>{ckpt_path}")
        save_path = os.path.join(cache_dir, os.path.basename(ckpt_path).replace('ckpt', 'npy'))
    if debug:
    ...
    for covis_k in covis_k_nums:
        ...
        if covis_k in cache_results and not overwrite:
            ...
            continue
        evaluator = MatcherEvaluator(vismatch=vismatch, ckpt_path=ckpt_path, oracle=oracle)
...
def run_benchmark(odir, root_dir='.', dataset_name='megadepth', split='test', p2d_type='sift',
                  ckpt=None, vismatch=False, oracle=False, merge_before_match=False, 
                  debug=False, overwrite=False, covis_k_nums=[1, 3, 5, 7, 10],
                  npts_max=1024, quant_desc=None, prefix=None):
    ...
    # Eval
    eval_model(data_loader, cache_dir, ckpt_path=ckpt, vismatch=vismatch, 
               oracle=oracle, debug=debug, overwrite=overwrite, 
               covis_k_nums=covis_k_nums, prefix=prefix)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--root_dir', type=str, default='.')
    parser.add_argument('--dataset', type=str, choices=['megadepth', '7scenes_sift_v2', '7scenes_superpoint_v2', 'cambridge', 'cambridge_sift'], default='megadepth')
    ...
    parser.add_argument('--p2d_type', type=str, choices=['sift', 'superpoint'], default='sift')
