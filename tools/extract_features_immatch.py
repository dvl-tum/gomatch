...
def compute_scene_im_features(detector, root_dir, dataset, split, dataset_conf):
    ...
    print(f"Extract features per scene, method={detector.name} cache dir={feature_cache_dir}")
    for sid in tqdm(sids_to_load, total=len(sids_to_load)):        
        ...
        for imid, im in scene_ims.items():
            if imid not in scene_features:
                ...
                descs = descs.cpu().data.numpy() if isinstance(descs, torch.Tensor) else descs
...
def extract_and_save(args):    
    detector, _ = init_model(config=args.immatch_config, benchmark_name='default', root_dir=args.root_dir)
