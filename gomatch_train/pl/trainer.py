...
class MatcherTrainer(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs) -> None:
        ...
        for k in ('R_err', 't_err'):
            ...
            quantile_errs = torch.quantile(errors_, quantile_ratios) if len(errors_) > 0 else [- 1.0] * 3
...
def train(args, exp_name):
    ...
    logger = TensorBoardLoggerMetrics(
        ...
        default_hp_metric=dict(
            R_err_q1=-1., t_err_q1=-1., R_err_q2=-1.,
            t_err_q2=-1., R_err_q3=-1., t_err_q3=-1., 
            failed=-1.,
        ),
    )
    ...
    # Training
    last_ckpt = os.path.join(args.odir, exp_name, args.resume_version, 'checkpoints/last.ckpt')
    if os.path.isfile(last_ckpt):
        ...
        trainer = pl.Trainer.from_argparse_args(
            args, logger=logger, deterministic=True, callbacks=callbacks,
