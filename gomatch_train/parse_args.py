...
def init_args():
    ...
    # Matcher args
    parser.add_argument(       
        '--matcher_class', type=str, default='OTMatcherCls',
        choices=['BPnPMatcher', 'OTMatcher', 'OTMatcherCls'],
    )
    parser.add_argument('--share_kp2d_enc', action='store_true')
    parser.add_argument('--att_layers', type=str, nargs='*', default=['self', 'cross', 'self'])    
    ...
    parser.add_argument('--p3d_type', choices=['coords', 'bvs', 'visdesc'], default='bvs')    
