...
def compute_metrics_sample(
    metrics, matches_est, matches_gt,
    pts2d, pts2d_pix, pts3d, 
    R_gt, t_gt, K,
    ransac_thres=0.001, is_test=False, print_out=True,
    iterations_count:int=1000, confidence:float=0.99,
):  
    ...
    pose_res = estimate_pose(
        pts2d[i2d, :2], pts3d[i3d], ransac_thres=ransac_thres, iterations_count=iterations_count, confidence=confidence
    )
    if not pose_res:
        R_err, t_err = -1., -1.
        inliers = []
    ...
    if pose_res:
        ...
        reproj_errs = reprojection_err(
            matches_gt_, pts2d_pix, pts3d, K, R, t
        ).cpu().data.numpy()
    else:
    ...
    if len(reproj_errs) == 0:
        if is_test:
            ...
            reproj_errs = np.array([-1.])
    reproj_errs = reproj_errs if is_test else reproj_errs.mean()
    metrics['reproj_gt3d_estpose'].append(reproj_errs)

    if print_out:        
        logger.info(f'i3d={len(i3d)} R_err={R_err:.2f} t_err={t_err:.2f} inls={len(inliers)} '
             f'Npts={len(pts2d)}/{len(pts3d)} n_matches={len(i3d)}')
    
    
def compute_metrics_batch(metrics, data, preds, cls=False, sc_thres=-1, 
                          ransac_thres=0.001, iterations_count:int=1000, confidence:float=0.99,
                          is_test=False, oracle=False,
                          debug=False):
    ...
    for bid in bids:
        ...
        compute_metrics_sample(
            ...
            iterations_count=iterations_count, confidence=confidence,
            is_test=is_test,
            print_out=debug
        )
    if is_test:
...
def summarize_metrics(
    ...
    # Query statis
    logger.info(f"Query total={n_queries} "
          f"evaluated={len(metrics.t_err)} "
          f"failed={failed}")
    
    # Match statis
    logger.info(f"Mean n2d={np.mean(metrics.n2d):.0f} "
    ...
    logger.info(f"R_qs={R_qs.data.numpy()} t_qs={t_qs.data.numpy()}m/{t_qs.data.numpy()*100}cm")
          
    # Localization recall
    localized = [
        100 * np.sum((np.array(metrics.t_err[pose_mask])< t_th) & (np.array(metrics.R_err[pose_mask]) < R_th)) / n_queries for t_th, R_th in pose_thresholds
    ]          
    logger.info("Localize recall@(<5cm5deg/0.25m2deg/0.5m5deg/1m10deg): "
