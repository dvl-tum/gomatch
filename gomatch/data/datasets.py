...
class BaseDataset(data.Dataset):
    ...
    def __init__(self, config, split='train'):
        ...
        with open(config.dataset_conf, 'r') as f:
            dataset_conf = Namespace(**yaml.load(f, Loader=yaml.FullLoader)[self.dataset])
            self._load_scene_data(dataset_conf)

    def _load_scene_data(self, dataset_conf):
        self.data_dir = data_dir = os.path.join(self.data_root, dataset_conf.data_processed_dir)
        ...
        self.feature_dir = os.path.join(data_dir, 'desc_cache', FEATURE_DIRS[self.p2d_type])
    ...
    def _construct_data(self, query, scene_pts3d, scene_ims):
        ...
        (
            pts3d, pts3dm, unmerge_mask, covis_ids
        ) = collect_covis_p3d_data(
            query, self.topk, scene_pts3d, scene_ims,
