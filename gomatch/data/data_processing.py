...
from gomatch.utils.geometry import points2d_to_bearing_vector, project_points3d, project3d_normalized
...
def load_scene_data(data_file, scenes, scene3d_file, feature_dir, load_desc=False):
    ...
    ims = {}
    num_pts3d = []
    logger.info(f'Fetching scene data for: {len(scenes)} scenes\nFeature dir:{feature_dir}')
    for sid in scenes:
        if sid not in data_dict:
            continue

        # Load pre-computed 2d feature
        scene_features = np.load(os.path.join(feature_dir, sid + '.npy'), allow_pickle=True).item()
    ...
    logger.info(f'Finished loading scenes: {len(pts3d_data)}, queries: {len(qids)} im.pts3d:{int(np.mean(num_pts3d))} #ims(pts3d <20)={(np.array(num_pts3d) < 20).sum()}')
    return sids, qids, ims, pts3d_data

def collect_covis_p3d_data(query, topk, pts_data, ims_data, p3d_type='bvs', 
                           npts=[10, 1024], merge_pts3dm=False, random_topk=False):
    npts_min, npts_max = npts
    topk_num = min(topk, len(query.topk))
    covis_im_ids = np.random.choice(query.topk, topk_num, replace=False) if random_topk else query.topk[:topk_num]
    ...
    if topk == 1:
        ...
        _, merge_mask, unmerge_mask = np.unique(db_pids, return_index=True, return_inverse=True)
...
def align_2d3d_points_normalized(pts2d, pts3d_proj, K, dist_thres, radial=None):    
    ...
    pts3d_proj_normed = points2d_to_bearing_vector(pts3d_proj, K, vec_dim=2, radial=radial)
    aligned_ids = align_points2d(
        pts2d_normed, pts3d_proj_normed, dist_thres=dist_thres
    )
    return aligned_ids

def compute_gt_2d3d_match(pts2d, pts3d, K, R, t, inls_thres=1, normalize=False, radial=None):
    ...
    if normalize:
        ...
        matches = align_2d3d_points_normalized(pts2d, pts2d_proj[valid], K, dist_thres=inls_thres, radial=radial) 
    else:
...
def enforce_outlier_rate_and_npts(ni, no2d, no3d, orate, npts=-1):
    ...
    nomax = int(ni * ormax / (1 - ormax )) if ormax >= 0 and ormax < 1 else max(no2d, no3d)
    no2d = min(no2d, nomax)
    no3d = min(no3d, nomax)
...
def subsample_points_indices(i2ds, i3ds, o2ds, o3ds, orate, npts=-1):
    ...
    ni_, no2d_, no3d_ = enforce_outlier_rate_and_npts(
        ni, no2d, no3d, orate, npts
    )
